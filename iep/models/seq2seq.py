...
# from torch.autograd import Va  riable
...
class Seq2Seq(nn.Module):
    ...
    def compute_loss(self, output_logprobs, y):
        ...
        Inputs:
        - output_logprobs: Vari able of shape (N, T_out, V_out)
        - y: LongTensor Vari able of shape (N, T_out)
        """
        self.multinomial_outputs = None
        V_in, V_out, D, H, L, N, T_in, T_out = self.get_dims(y=y)
    ...
    def reinforce_sample(self, x, max_length=30, temperature=1.0, argmax=False):
        ...
        for t in range(T):
            ...
            if done.sum() == N:
                break
        # return Va riable(y.type_as(x.data))
